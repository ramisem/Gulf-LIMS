...
from .models import AccessionTemplate, PathologyTemplate, BioPharmaAccessionTemplate, Macros
...
from masterdata.models import Client, BodySite, BodySubSiteMap, BioProject, Sponsor, BioSite, ProjectVisitMap, \
    AccessionPrefix
from tests.models import ICDCode
from accessioning.models import Accession
from django.db import models, transaction
from .widgets import LookupWidget, NextLinkWidget, FinishLinkWidget, PrevLinkNextLinkWidget, \
    MagnifyingTextInput, \
    PartNoInputWidget
from django.forms import BaseInlineFormSet
from analysis.forms import CustomSummernoteWidget



# Form for the Accession Page
class AccessionTemplateForm(forms.ModelForm):
    ...
    case_id = forms.CharField(label='Accession Template ID', max_length=40, required=False)
    ...
    move_prev_next_from_reporting_doc_tab = forms.CharField(
        widget=PrevLinkNextLinkWidget(prev_tab='client-tab', next_tab='sample-creation-tab'), required=False, label=" ")
    ...
    reporting_type = forms.CharField(label='Reporting Type', max_length=40, required=False,
                                     widget=forms.HiddenInput())
    is_template = forms.CharField(widget=forms.HiddenInput(), required=False)
    count_accession = forms.CharField(widget=forms.HiddenInput(), label='Count Accession', max_length=40,
                                      required=False,
                                      )
    ...
    def clean(self):
        ...
        if not case_id:
            self.add_error('case_id', "Accession Template ID is required")
    ...
    def __init__(self, *args, request=None, **kwargs):
        ...
        if request and not self.instance.pk:  # Only set during creation, not editing
            self.fields['is_template'].initial = "True"
        ...
        self.fields["accession_category"] = forms.ChoiceField(
            choices=[(None, "Select an Option")] + UtilClass.get_refvalues_for_field(required_reftype),
            required=False, label="Accession Category",
            widget=forms.Select(attrs={'onchange': 'onChangeAccessionCategory()'}),
        )
        required_reftype = getattr(settings, 'APPLICATION_ACCESSION_TEMPLATE_REFERENCE', '')
        self.fields["accession_template"] = forms.ChoiceField(
            choices=[(None, "Select an Option")] + UtilClass.get_refvalues_for_field(required_reftype),
            required=False, label="Accession Template"
        )
        required_reftype = getattr(settings, 'APPLICATION_ACCESSION_PREFIX_REFERENCE', '')
        ...
        if self.instance.accession_id is not None:
            ...
            accession_count = Accession.objects.filter(accession_template=self.instance.accession_id).count()
            self.fields['count_accession'].widget.attrs['value'] = str(accession_count)
            self.fields['hidden_accession_prefix'].initial = self.instance.accession_prefix

    class Meta:
        model = AccessionTemplate
        fields = ['patient_id', 'insurance_id', 'client_id', 'doctor', 'accession_id', 'case_id',
                  ...
                  'hidden_accession_prefix', 'receive_dt_timezone', 'accession_type']
        widgets = {
...
class BioPharmaAccessionTemplateForm(AccessionTemplateForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.data:
            try:
                ...
                pass  # Handle cases where the data is not yet a valid ID

    class Meta(AccessionTemplateForm.Meta):
        model = BioPharmaAccessionTemplate
        fields = AccessionTemplateForm.Meta.fields + ['project', 'sponsor', 'investigator', 'visit']

        widgets = {
            'sponsor': forms.Select(attrs={
                'onchange': 'sponsorChanged(this)',  # Trigger for sponsor change
                'class': 'admin-autocomplete form-control select2',
                'style': 'width: 100%',
            }),
            'project': forms.Select(attrs={
                'onchange': 'projectChanged(this)',  # Trigger for project change
                'class': 'admin-autocomplete form-control select2',
                'style': 'width: 100%',
            }),
            # You can also add widgets for visit and investigator if they need styling
...
class SampleInlineForm(forms.ModelForm):
    ...
    def clean_part_no(self):
        ...
        accession_category = AccessionTemplate.objects.get(
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.sample_id is not None:
            ...
            self.fields["body_site"] = forms.ChoiceField(
                ...
                required=False,
                widget=forms.Select(attrs={'onchange': 'populateSubSite(this)'})
            )
...
class AccessionICDCodeMapForm(forms.ModelForm):
    description = forms.CharField(label='Description', max_length=40, required=False,
                                  widget=forms.TextInput(attrs={'readonly': 'readonly'}))
...
class PathologyTemplateForm(forms.ModelForm):
    class Meta:
...
class MacrosForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        required_reftype = getattr(settings, 'APPLICATION_MACROS_TYPE_REFERENCE', '')

        self.fields["macros_type"] = forms.ChoiceField(
            choices=[(None, "Select an Option")] +
                    UtilClass.get_refvalues_for_field(required_reftype),
            required=False,
            label="Macros Type"
        )