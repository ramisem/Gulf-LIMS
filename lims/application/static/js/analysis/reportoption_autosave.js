(function () {
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function getRowKey($row, index) {
            return $row.find('input[name$="-hidden_report_option_dtl_id"]').val() || `row-${index}`;
        }
        ...
        function getAnalyteValue($row) {
            ...
            if ($valEl.is('select')) {
                return $valEl.find('option:selected').text().trim();
            }
        ...
        function triggerAutosave($row) {
            const data = {
                reportoptiondtlid: $row.find('input[name$="-hidden_report_option_dtl_id"]').val(),
                reportoption: $row.find('input[name$="-report_option_id"]').val(),
        ...
        function handleAnalyteChange($changedRow) {
            ...
            if (currentValue !== prevValue) {
                ...
                triggerAutosave($changedRow);  // keep autosave

                const changedAnalyte = $changedRow.find('td.field-analyte_id a').text().trim();

                $('tr.dynamic-reportoptiondtl_set').each(function () {
                    const $row = $(this);
                    const rawLookups = $row.find('[data-dropdown-lookups]').attr('data-dropdown-lookups') || '';
                    const lookups = rawLookups.split(';').map(s => s.trim()).filter(Boolean);

                    if (lookups.includes(changedAnalyte)) {
                        $row.data('dep-changed', true);
                        processRow($row, currentValue, true); // immediately process and reload
                    }
                });
            }
        }
        ...
        function registerChangeListeners() {
            // Inputs & selects
            $('tr.dynamic-reportoptiondtl_set td.field-analyte_value :input').on('change', function () {
                const $row = $(this).closest('tr.dynamic-reportoptiondtl_set');
            ...
            $('iframe').each(function () {
                ...
                if (editable) {
                    editable.addEventListener('input', function () {
                        const $row = $(iframe).closest('tr.dynamic-reportoptiondtl_set');
                        handleAnalyteChange($row);
                    });
                }
            });
        ...
        $(function () {
            ...
            $('tr.dynamic-reportoptiondtl_set').each(function (i) {
        ...
        function autosaveOnExit() {
            ...
            $('tr.dynamic-reportoptiondtl_set').each(function () {
                ...
                form.append('reportoptiondtlid', $row.find('input[name$="-hidden_report_option_dtl_id"]').val());
                form.append('reportoption', $row.find('input[name$="-report_option_id"]').val());
        ...
        // Focus handlers
        $(document).on('focus', 'tr.dynamic-reportoptiondtl_set textarea', function () {
            this.setSelectionRange(0, 0);
        });

        $(document).on('focus', 'tr.dynamic-reportoptiondtl_set .note-editing-area textarea', function () {
        ...
        $(document).on('keydown', 'tr.dynamic-reportoptiondtl_set textarea', function (e) {
            if (e.key === 'Tab') {
        ...
        function setupSummernoteFocusAndChange() {
            ...
            setTimeout(() => {
                $('tr.dynamic-reportoptiondtl_set td.field-analyte_value iframe').each(function () {
                    const iframe = this;
        ...
        function processRow($row, changedValue, reloadAfter = false) {
            ...
            lookups.forEach(analyteName => {
                ...
                const $relatedRow = $('tr.dynamic-reportoptiondtl_set').filter(function () {
            ...
            console.log('TRIGGER: analyte fetch', { row: $row.attr('id'), taId, reportoption, params });
            ...
            $.ajax({
                ...
                success(res) {
                    ...
                    if (reloadAfter) {
                        ...
                        }, 500);  // 300ms delay gives time to update DOM/input fields
                    }
                }
            });
        }
