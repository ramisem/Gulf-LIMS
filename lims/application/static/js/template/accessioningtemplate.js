...
$(document).ready(function () {
    ...
    const updateAccPrefixBtn = document.getElementById("btn-update-acc-prefix");
    if (updateAccPrefixBtn) {
        updateAccPrefixBtn.style.display="none";
    }
    ...
    let saveContinueButton = document.querySelector('input[name="_continue"]');
...
function populateSponsorDetails() {
    console.log("Sponsor dropdown changed! Firing populateSponsorDetails...");
...
function populateRelatedTest(el) {
    ...
    if (!bodySiteSelect || !testIdElement || !testCodeElement) {
        console.error('Cannot find Body Site or Test Id or Test Name');
        return;
    }
    ...
    if (bodySiteValue && bodySiteValue !== 'undefined') {
        const inputURL = window.location.protocol + "//" + window.location.host + '/masterdata/ajax/get-tests-based-on-bodysites/';
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                response.tests.forEach(function(item) {
                    str_test_id = str_test_id + "," + item.test_id;
                    str_test_name = str_test_name + "," + item.test_name;
                });
                existing_test_id = testIdElement.value;
                existing_test_name = testCodeElement.value;
                if(existing_test_id!=null && existing_test_id!=""){
                    str_test_id = existing_test_id + str_test_id;
                }
                if(existing_test_name!=null && existing_test_name!=""){
                    str_test_name = existing_test_name + str_test_name;
                }
                if(str_test_id!=null && str_test_id!=""){
                    if (str_test_id.startsWith(',')) {
                        str_test_id = str_test_id.substring(1);
                    }
                    testIdElement.value = str_test_id;
                }
                if(str_test_name!=null && str_test_name!=""){
                    if (str_test_name.startsWith(',')) {
                        str_test_name = str_test_name.substring(1);
                    }
